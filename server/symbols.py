...
USING_ROLES = frozenset({"rule_body", "constraint", "minimize", "maximize", "weak", "external", "heuristic"})
DIRECTIVE_ROLES = frozenset({"show", "minimize", "maximize"})
...
def collect_occurrences(tree: lark.Tree | None) -> list[Occurrence]:
    ...
    for statement_wrapper in tree.children:
        ...
        elif statement.data == "maximize_directive":
            for child in statement.children:
                _collect_atoms_with_role(child, "maximize", occurrences)
        ...
        elif statement.data == "heuristic_directive":
            _collect_atoms_with_role(statement.children[0], "heuristic", occurrences)
            if len(statement.children) > 1 and isinstance(statement.children[1], lark.Tree):
                _collect_atoms_with_role(statement.children[1], "heuristic", occurrences)
